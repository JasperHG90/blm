...
k <- 50000
# Fit the model
fit <- blm("Compensation ~ Age + Male + Sector", data=directors) %>%
  ...
  # Set hypotheses
  # H1: Males earn more than females
  ...
  set_hypothesis("b0 + b4 & b0 + b4 > b0 & b0 > b0 + b3") %>%
  # Sample posterior
  sample_posterior()
...
fit$hypotheses$hypothesis_1$result
fit$hypotheses$hypothesis_2$result
fit$hypotheses$hypothesis_3$result
