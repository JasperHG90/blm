...
dir_jags <- with(directors, list(# Outcome for individuals (level 1)
                                 ...
                                 sector=as.numeric(as.factor(Company)),
...
if(!"model1.rds" %in% list.files("experiments/4Multilevel")) {
...
# Variance explained
vl2 <- MAPm1[3]^2 / sum(MAPm1[-1]^2) # 57.4%
vl1 <- 1-vl2 # 42.6%
```
...
if(!"model1.rds" %in% list.files("experiments/4Multilevel")) {
  ...
  resm2 <- readRDS(file="model2.rds")
  DICm2 <- readRDS(file="model2_DIC.rds")  
...
# Specify model in JAGS
mod_io <- jags.model("2b_level_one_predictors.txt", 
                     data = dir_jags,
                     inits = dir_inits,
                     n.chains=2)
...
# Run the chain
resm2b <- coda.samples(mod_io, variable.names = params, n.iter=500000, thin = 10)

# MAP values
MAPm2b <- apply(do.call(rbind.data.frame, resm2b), 2, mean)

# DIC
DICm2b <- dic.samples(mod_io, thin=5, n.iter=10000)
```
...
# Draw samples
params <- c("sigma_e", "sigma_u0", "gamma_00", "gamma_10", "gamma_01", "gamma_02")
# Run the chain
