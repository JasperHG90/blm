...
```{r model 3 (by company)}
dirmod3 <- blm("Compensation ~ Company + Age + Male",
                data=directors) 

# Retrieve parameter names
dirmod3 %>% get_parameter_names()
...
dirmod3 <- dirmod3 %>%
  ...
  delete_posterior() %>%
  # Set an informative prior on the coefficients
  # Males are expected to have 17% pay increase over women
  #set_prior("b53", mu = .17, sd=0.01) %>%
  # Update sampling settings
  set_sampling_options(chains = 2, iterations = 10000,
                       burn = 3000, thinning=1) %>%
  # Set initial values
  # If we draw from the priors the starting values will be too large
  # This is an issue for MH because it takes many more iterations to converge
  set_initial_values(chain_1 = list("b" = runif(54, 1, 10), "sigma"= 1),
                     chain_2 = list("b" = runif(54, 1, 10), "sigma"=2)) %>%
  # Sample the posterior distribution
  sample_posterior()
