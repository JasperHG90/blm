---
title: 'The Effect of Gender on Director Compensation'
...
In accordance with the available evidence, we hypothesize that male directors earn more than their female counterparts and that compensation only increases with age. $H_u$ is the unconstrained hypothesis.

$$
\text{H}_1\text{: } \beta_{\text{Male}} > 0 \ \ \ \ \ \ \ \text{H}_2\text{: } \beta_{\text{Age}} > 0 \ \ \ \ \ \ \ \text{H}_u\text{: } \beta_\text{Male}, \beta_{\text{Age}}
$$
...
dirmod2 <- dirmod2 %>%
  ...
  compute_null_model() %>%
  # Set hypotheses
  set_hypothesis("H1", "b2 > 0") %>% # Gender: larger than 0
...
# Compare hypotheses with Bain
library(bain)
## Compared to Bain
regr <- lm(Compensation ~ Age + Male, directors)
# Set seed
summary(regr)
z<-bain(regr,"Male > 0; Age > 0", standardize = FALSE)
z # This bayes factor differs because we set a different prior ==> therefore the complexity is not the same.
