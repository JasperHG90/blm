...
Most of the literature on director compensation concerns execute directors. Execute directors are employed by the firm, while independent directors are concerned primarily with oversight.
...
We hypothesize that there is at most a small difference between the compensation of male and female directors.
...
Then the unconstrained hypothesis is:
...
dirmod2 <- dirmod2 %>%
  ...
  compute_null_model() %>%
  # Set hypotheses
  set_hypothesis("H1", "|b0 - (b0 + b4)| < .01") %>% # Gender
  set_hypothesis("H2", "b3 > 0") %>%                # Age
  # Sample the posterior distribution
  sample_posterior()
...
dirmod2 %>%
  ...
  get_value("DIC") %>%
  summary() # ==> 512 so the model above fits significantly better
